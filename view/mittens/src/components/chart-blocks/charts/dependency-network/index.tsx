...
export default function DependencyNetwork() {
    ...
    const selectedNodeLinkIds = useMemo(() => {
        ...
        return ids;
    }, [selectedNode, networkData.links]);
  ...
  const handleNodeClick = (node: D3Node | null) => {
    ...
    useEffect(() => {
        ...
        const update = () =>
            setDetailsHeight(
                Math.max(0, Math.floor(el.getBoundingClientRect().height))
            );
        update();
    ...
    const getStatusColor = (node: D3Node) => {
        if (node.errorInfo.isPartOfCycle)
            return "text-purple-600 dark:text-purple-400 bg-purple-100 dark:bg-purple-400/20";
        if (node.errorInfo.hasErrors) {
            switch (node.errorInfo.errorSeverity) {
                case "ERROR":
                    return "text-red-600 dark:text-red-400 bg-red-100 dark:bg-red-400/20";
                case "WARNING":
                    return "text-yellow-600 dark:text-yellow-400 bg-yellow-100 dark:bg-yellow-400/20";
                default:
    ...
    const getComponentType = (node: D3Node): string => {
        ...
        if (
            packageName.includes("repositories") ||
            packageName.includes("repo")
        )
            return "repo";
        if (packageName.includes("payment")) return "payment";
        if (
            packageName.includes("main") ||
            node.className.toLowerCase().includes("main")
        )
            return "main";
        return "other";
    };
    ...
    const getConnectionCount = (nodeId: string) => {
        return networkData.links.filter(
            (link) =>
                (typeof link.source === "string"
                    ? link.source
                    : (link.source as D3Node).id) === nodeId ||
                (typeof link.target === "string"
                    ? link.target
                    : (link.target as D3Node).id) === nodeId
        ).length;
    };
    ...
    const providerTree = useMemo(() => {
        ...
        const nodeMap = Object.fromEntries(
            networkData.nodes.map((n) => [n.id, n])
        );
        // Map providerId → array of providee nodes
        const providesLinks = networkData.links.filter(
            (l) => l.type === "PROVIDES"
        );
        ...
        providesLinks.forEach((link) => {
            ...
            const provideeId =
                typeof link.target === "string"
                    ? link.target
                    : (link.target as D3Node).id;
            if (!providerToProvidees[providerId])
                providerToProvidees[providerId] = [];
            if (nodeMap[provideeId])
                providerToProvidees[providerId].push(nodeMap[provideeId]);
        });
        // Find root nodes (nodes that are not providees)
        const provideeIds = new Set(
            providesLinks.map((l) =>
                typeof l.target === "string"
                    ? l.target
                    : (l.target as D3Node).id
            )
        );
    ...
    const matchesFilters = (node: D3Node): boolean => {
        ...
        const matchesType =
            ...
            getComponentType(node) === componentTypeFilter;
        return matchesType;
    };
    ...
    const hasMatchingDescendant = (node: D3Node): boolean => {
        ...
        return providees.some(
            (child) => matchesFilters(child) || hasMatchingDescendant(child)
        );
    };

    // Filter the tree roots to show only nodes that match or have matching descendants
    const filteredRoots = useMemo(() => {
        return providerTree.roots.filter(
            (node) => matchesFilters(node) || hasMatchingDescendant(node)
        );
    ...
    const renderTreeRows: any = (node: D3Node, depth = 0) => {
        ...
        const hasProvidees =
            providerTree.providerToProvidees[node.id]?.length > 0;
        return (
                ...
                >
                    ...
                    <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-gray-900 dark:text-white font-medium group-hover:text-gray-700 dark:group-hover:text-gray-200">
                                {node.label}
                            </span>
                        </div>
                    </td>
                    ...
                    <td className="py-4 px-6 text-center">
                        <span
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all duration-200 shadow-sm ${statusColor}`}
                        >
                            {statusText}
                        </span>
                    </td>
        ...
        <div className="w-full">
                    ...
                    <div className="text-3xl font-bold text-red-600 dark:text-red-400 mb-1">
                        {
                            networkData.nodes.filter(
                                (n) =>
                                    n.errorInfo.hasErrors &&
                                    n.errorInfo.errorSeverity === "ERROR"
                            ).length
                        }
                    </div>
            ...
            <div className="mb-6 flex flex-col lg:flex-row gap-4 items-stretch">
                ...
                <div
                    ...
                    style={{
                        height:
                            selectedNode && detailsHeight ? detailsHeight : 650,
                        minHeight: 650,
                    }}
                >
                    <D3Network
                ...
                {selectedNode && (
                    <div
                        ref={detailsRef}
                        className="lg:w-96 bg-gray-50 dark:bg-slate-700 border border-gray-200 dark:border-slate-600 rounded-lg p-4 self-start"
                    >
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            ...
                            <div>
                                ...
                                <p className="text-gray-900 dark:text-white font-medium">
                                    {selectedNode.metadata.dependencyCount}
                                </p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-600 dark:text-gray-400">
                                ...
                                <p className="text-gray-900 dark:text-white font-medium">
                                    {selectedNode.metadata.providerCount}
                                </p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-600 dark:text-gray-400">
                            ...
                            {selectedNode.errorInfo.hasErrors && (
                                <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
                                    ...
                                    <div className="border-t border-red-200 dark:border-red-800 pt-3">
                                        ...
                                        <div className="bg-red-100 dark:bg-red-900/30 rounded-md p-3 border-l-4 border-red-400 overflow-x-auto max-w-full">
                                            {networkData.errorContext.issueDetails
                                                // Scope to issues both of the same type AND affecting the selected node
                                                .filter(
                                                    (issue) =>
                                                        selectedNode.errorInfo.errorTypes.includes(
                                                            issue.type
                                                        ) &&
                                                ...
                                                .map((issue, index) => (
                                                    <p
                                                        key={index}
                                                        className="text-red-700 dark:text-red-300 text-sm leading-relaxed mb-2 last:mb-0 whitespace-pre-wrap break-words font-mono"
                                                    >
                                                        {issue.suggestedFix}
                                                    </p>
                                                ))}
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3 lg:items-center">
                            ...
                            <div className="flex gap-1 flex-nowrap">
                                ...
                                >
                                    <span>
                                        All ({networkData.nodes.length})
                                    </span>
                                </button>

                                <button
                                    onClick={() =>
                                        setComponentTypeFilter("main")
                                    }
                                    className={`flex items-center gap-1.5 px-2.5 py-2 text-xs font-semibold rounded-lg transition-all duration-200 shadow-sm whitespace-nowrap ${
                                ...
                                <button
                                    onClick={() =>
                                        setComponentTypeFilter("services")
                                    }
                                    className={`flex items-center gap-1.5 px-2.5 py-2 text-xs font-semibold rounded-lg transition-all duration-200 shadow-sm whitespace-nowrap ${
                                ...
                                >
                                    <div className="w-3 h-3 rounded-full bg-amber-500 flex items-center justify-center">
                                        ...
                                        >
                                            <path
                                                fillRule="evenodd"
                                                d="M6 6V5a3 3 0 013-3h2a3 3 0 013 3v1h2a2 2 0 012 2v6a2 2 0 01-2 2H4a2 2 0 01-2-2V8a2 2 0 012-2h2zm2-1a1 1 0 011-1h2a1 1 0 011 1v1H8V5zm1 5a1 1 0 011-1h.01a1 1 0 010 2H10a1 1 0 01-1-1z"
                                                clipRule="evenodd"
                                            />
                                        </svg>
                                    </div>
                                ...
                                <button
                                    onClick={() =>
                                        setComponentTypeFilter("payment")
                                    }
                                    className={`flex items-center gap-1.5 px-2.5 py-2 text-xs font-semibold rounded-lg transition-all duration-200 shadow-sm whitespace-nowrap ${
